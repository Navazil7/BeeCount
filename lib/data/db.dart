...
/// Agent 本地显式记忆。此表不参与 LocalChanges 或任何云同步载荷。
...
/// 账本范围内的对话摘要，仅用于下一次本地 Agent 上下文拼装。
class AgentConversationSummaries extends Table {
  IntColumn get id => integer().autoIncrement()();
  IntColumn get ledgerId => integer().nullable()();
...
/// Agent 单次运行的本地审计状态。runId 由客户端生成，绝不上传。
...
/// 工具调用审计。复合唯一约束提供同一 run 内的幂等写入保障。
class AgentToolCalls extends Table {
  ...
  @override
  List<String> get customConstraints => const ['UNIQUE(run_id, call_id)'];
}
...
class BeeDatabase extends _$BeeDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
    ...
    WHERE currency_code IS NULL;''');
          ...
          if (from < 33) {
            logger.info('DBMigration', '开始迁移到 v33: Agent 本地记忆与审计');
            await _createTableIfMissing(
                migrator, 'agent_memories', agentMemories);
            await _createTableIfMissing(
              migrator,
              'agent_conversation_summaries',
              agentConversationSummaries,
            );
            await _createTableIfMissing(migrator, 'agent_runs', agentRuns);
            await _createTableIfMissing(
              migrator,
              'agent_tool_calls',
              agentToolCalls,
  ...
  Future<void> _createAgentMemoryFtsTable() => customStatement(
